...
function VimCmdLineStartApp()
    ...
    execute 'nnoremap <silent><buffer><space>' . g:cmdline_visual_send_line_mapping . '<space>:call VimCmdLineSendLine()<CR>'
    if exists("b:cmdline_source_fun")
        execute 'vnoremap <silent><buffer><space>' . g:cmdline_visual_send_line_mapping . '<esc><space>:call ' . b:cmdline_source_fun(getline("'<", "'>")) . '<CR>'
        execute 'nnoremap <silent><buffer><space>' . g:cmdline_send_file_mapping . '<space>:call ' . b:cmdline_source_fun(getline(1, "$")) . '<CR>'
        execute 'nnoremap <silent><buffer><space>' . g:cmdline_send_paragraph_mapping . '<space>:call VimCmdLineSendParagraph()<CR>'
        execute 'nnoremap <silent><buffer><space>' . g:cmdline_send_marked_block_mapping . '<space>:call VimCmdLineSendMBlock()<CR>'
    endif
    if exists("b:cmdline_quit_cmd")
        execute 'nnoremap <silent><buffer><space>' . g:cmdline_quit_mapping . '<space>:call VimCmdLineQuit()<CR>'
    endif
